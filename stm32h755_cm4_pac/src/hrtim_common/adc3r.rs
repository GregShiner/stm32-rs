#[doc = "Register `ADC3R` reader"]
pub type R = crate::R<Adc3rSpec>;
#[doc = "Register `ADC3R` writer"]
pub type W = crate::W<Adc3rSpec>;
#[doc = "Field `AD1MC1` reader - AD1MC1"]
pub type Ad1mc1R = crate::BitReader;
#[doc = "Field `AD1MC1` writer - AD1MC1"]
pub type Ad1mc1W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1MC2` reader - AD1MC2"]
pub type Ad1mc2R = crate::BitReader;
#[doc = "Field `AD1MC2` writer - AD1MC2"]
pub type Ad1mc2W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1MC3` reader - AD1MC3"]
pub type Ad1mc3R = crate::BitReader;
#[doc = "Field `AD1MC3` writer - AD1MC3"]
pub type Ad1mc3W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1MC4` reader - AD1MC4"]
pub type Ad1mc4R = crate::BitReader;
#[doc = "Field `AD1MC4` writer - AD1MC4"]
pub type Ad1mc4W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1MPER` reader - AD1MPER"]
pub type Ad1mperR = crate::BitReader;
#[doc = "Field `AD1MPER` writer - AD1MPER"]
pub type Ad1mperW<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1EEV1` reader - AD1EEV1"]
pub type Ad1eev1R = crate::BitReader;
#[doc = "Field `AD1EEV1` writer - AD1EEV1"]
pub type Ad1eev1W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1EEV2` reader - AD1EEV2"]
pub type Ad1eev2R = crate::BitReader;
#[doc = "Field `AD1EEV2` writer - AD1EEV2"]
pub type Ad1eev2W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1EEV3` reader - AD1EEV3"]
pub type Ad1eev3R = crate::BitReader;
#[doc = "Field `AD1EEV3` writer - AD1EEV3"]
pub type Ad1eev3W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1EEV4` reader - AD1EEV4"]
pub type Ad1eev4R = crate::BitReader;
#[doc = "Field `AD1EEV4` writer - AD1EEV4"]
pub type Ad1eev4W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1EEV5` reader - AD1EEV5"]
pub type Ad1eev5R = crate::BitReader;
#[doc = "Field `AD1EEV5` writer - AD1EEV5"]
pub type Ad1eev5W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TAC2` reader - AD1TAC2"]
pub type Ad1tac2R = crate::BitReader;
#[doc = "Field `AD1TAC2` writer - AD1TAC2"]
pub type Ad1tac2W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TAC3` reader - AD1TAC3"]
pub type Ad1tac3R = crate::BitReader;
#[doc = "Field `AD1TAC3` writer - AD1TAC3"]
pub type Ad1tac3W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TAC4` reader - AD1TAC4"]
pub type Ad1tac4R = crate::BitReader;
#[doc = "Field `AD1TAC4` writer - AD1TAC4"]
pub type Ad1tac4W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TAPER` reader - AD1TAPER"]
pub type Ad1taperR = crate::BitReader;
#[doc = "Field `AD1TAPER` writer - AD1TAPER"]
pub type Ad1taperW<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TARST` reader - AD1TARST"]
pub type Ad1tarstR = crate::BitReader;
#[doc = "Field `AD1TARST` writer - AD1TARST"]
pub type Ad1tarstW<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TBC2` reader - AD1TBC2"]
pub type Ad1tbc2R = crate::BitReader;
#[doc = "Field `AD1TBC2` writer - AD1TBC2"]
pub type Ad1tbc2W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TBC3` reader - AD1TBC3"]
pub type Ad1tbc3R = crate::BitReader;
#[doc = "Field `AD1TBC3` writer - AD1TBC3"]
pub type Ad1tbc3W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TBC4` reader - AD1TBC4"]
pub type Ad1tbc4R = crate::BitReader;
#[doc = "Field `AD1TBC4` writer - AD1TBC4"]
pub type Ad1tbc4W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TBPER` reader - AD1TBPER"]
pub type Ad1tbperR = crate::BitReader;
#[doc = "Field `AD1TBPER` writer - AD1TBPER"]
pub type Ad1tbperW<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TBRST` reader - AD1TBRST"]
pub type Ad1tbrstR = crate::BitReader;
#[doc = "Field `AD1TBRST` writer - AD1TBRST"]
pub type Ad1tbrstW<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TCC2` reader - AD1TCC2"]
pub type Ad1tcc2R = crate::BitReader;
#[doc = "Field `AD1TCC2` writer - AD1TCC2"]
pub type Ad1tcc2W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TCC3` reader - AD1TCC3"]
pub type Ad1tcc3R = crate::BitReader;
#[doc = "Field `AD1TCC3` writer - AD1TCC3"]
pub type Ad1tcc3W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TCC4` reader - AD1TCC4"]
pub type Ad1tcc4R = crate::BitReader;
#[doc = "Field `AD1TCC4` writer - AD1TCC4"]
pub type Ad1tcc4W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TCPER` reader - AD1TCPER"]
pub type Ad1tcperR = crate::BitReader;
#[doc = "Field `AD1TCPER` writer - AD1TCPER"]
pub type Ad1tcperW<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TDC2` reader - AD1TDC2"]
pub type Ad1tdc2R = crate::BitReader;
#[doc = "Field `AD1TDC2` writer - AD1TDC2"]
pub type Ad1tdc2W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TDC3` reader - AD1TDC3"]
pub type Ad1tdc3R = crate::BitReader;
#[doc = "Field `AD1TDC3` writer - AD1TDC3"]
pub type Ad1tdc3W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TDC4` reader - AD1TDC4"]
pub type Ad1tdc4R = crate::BitReader;
#[doc = "Field `AD1TDC4` writer - AD1TDC4"]
pub type Ad1tdc4W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TDPER` reader - AD1TDPER"]
pub type Ad1tdperR = crate::BitReader;
#[doc = "Field `AD1TDPER` writer - AD1TDPER"]
pub type Ad1tdperW<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TEC2` reader - AD1TEC2"]
pub type Ad1tec2R = crate::BitReader;
#[doc = "Field `AD1TEC2` writer - AD1TEC2"]
pub type Ad1tec2W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TEC3` reader - AD1TEC3"]
pub type Ad1tec3R = crate::BitReader;
#[doc = "Field `AD1TEC3` writer - AD1TEC3"]
pub type Ad1tec3W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TEC4` reader - AD1TEC4"]
pub type Ad1tec4R = crate::BitReader;
#[doc = "Field `AD1TEC4` writer - AD1TEC4"]
pub type Ad1tec4W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `AD1TEPER` reader - AD1TEPER"]
pub type Ad1teperR = crate::BitReader;
#[doc = "Field `AD1TEPER` writer - AD1TEPER"]
pub type Ad1teperW<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    #[doc = "Bit 0 - AD1MC1"]
    #[inline(always)]
    pub fn ad1mc1(&self) -> Ad1mc1R {
        Ad1mc1R::new((self.bits & 1) != 0)
    }
    #[doc = "Bit 1 - AD1MC2"]
    #[inline(always)]
    pub fn ad1mc2(&self) -> Ad1mc2R {
        Ad1mc2R::new(((self.bits >> 1) & 1) != 0)
    }
    #[doc = "Bit 2 - AD1MC3"]
    #[inline(always)]
    pub fn ad1mc3(&self) -> Ad1mc3R {
        Ad1mc3R::new(((self.bits >> 2) & 1) != 0)
    }
    #[doc = "Bit 3 - AD1MC4"]
    #[inline(always)]
    pub fn ad1mc4(&self) -> Ad1mc4R {
        Ad1mc4R::new(((self.bits >> 3) & 1) != 0)
    }
    #[doc = "Bit 4 - AD1MPER"]
    #[inline(always)]
    pub fn ad1mper(&self) -> Ad1mperR {
        Ad1mperR::new(((self.bits >> 4) & 1) != 0)
    }
    #[doc = "Bit 5 - AD1EEV1"]
    #[inline(always)]
    pub fn ad1eev1(&self) -> Ad1eev1R {
        Ad1eev1R::new(((self.bits >> 5) & 1) != 0)
    }
    #[doc = "Bit 6 - AD1EEV2"]
    #[inline(always)]
    pub fn ad1eev2(&self) -> Ad1eev2R {
        Ad1eev2R::new(((self.bits >> 6) & 1) != 0)
    }
    #[doc = "Bit 7 - AD1EEV3"]
    #[inline(always)]
    pub fn ad1eev3(&self) -> Ad1eev3R {
        Ad1eev3R::new(((self.bits >> 7) & 1) != 0)
    }
    #[doc = "Bit 8 - AD1EEV4"]
    #[inline(always)]
    pub fn ad1eev4(&self) -> Ad1eev4R {
        Ad1eev4R::new(((self.bits >> 8) & 1) != 0)
    }
    #[doc = "Bit 9 - AD1EEV5"]
    #[inline(always)]
    pub fn ad1eev5(&self) -> Ad1eev5R {
        Ad1eev5R::new(((self.bits >> 9) & 1) != 0)
    }
    #[doc = "Bit 10 - AD1TAC2"]
    #[inline(always)]
    pub fn ad1tac2(&self) -> Ad1tac2R {
        Ad1tac2R::new(((self.bits >> 10) & 1) != 0)
    }
    #[doc = "Bit 11 - AD1TAC3"]
    #[inline(always)]
    pub fn ad1tac3(&self) -> Ad1tac3R {
        Ad1tac3R::new(((self.bits >> 11) & 1) != 0)
    }
    #[doc = "Bit 12 - AD1TAC4"]
    #[inline(always)]
    pub fn ad1tac4(&self) -> Ad1tac4R {
        Ad1tac4R::new(((self.bits >> 12) & 1) != 0)
    }
    #[doc = "Bit 13 - AD1TAPER"]
    #[inline(always)]
    pub fn ad1taper(&self) -> Ad1taperR {
        Ad1taperR::new(((self.bits >> 13) & 1) != 0)
    }
    #[doc = "Bit 14 - AD1TARST"]
    #[inline(always)]
    pub fn ad1tarst(&self) -> Ad1tarstR {
        Ad1tarstR::new(((self.bits >> 14) & 1) != 0)
    }
    #[doc = "Bit 15 - AD1TBC2"]
    #[inline(always)]
    pub fn ad1tbc2(&self) -> Ad1tbc2R {
        Ad1tbc2R::new(((self.bits >> 15) & 1) != 0)
    }
    #[doc = "Bit 16 - AD1TBC3"]
    #[inline(always)]
    pub fn ad1tbc3(&self) -> Ad1tbc3R {
        Ad1tbc3R::new(((self.bits >> 16) & 1) != 0)
    }
    #[doc = "Bit 17 - AD1TBC4"]
    #[inline(always)]
    pub fn ad1tbc4(&self) -> Ad1tbc4R {
        Ad1tbc4R::new(((self.bits >> 17) & 1) != 0)
    }
    #[doc = "Bit 18 - AD1TBPER"]
    #[inline(always)]
    pub fn ad1tbper(&self) -> Ad1tbperR {
        Ad1tbperR::new(((self.bits >> 18) & 1) != 0)
    }
    #[doc = "Bit 19 - AD1TBRST"]
    #[inline(always)]
    pub fn ad1tbrst(&self) -> Ad1tbrstR {
        Ad1tbrstR::new(((self.bits >> 19) & 1) != 0)
    }
    #[doc = "Bit 20 - AD1TCC2"]
    #[inline(always)]
    pub fn ad1tcc2(&self) -> Ad1tcc2R {
        Ad1tcc2R::new(((self.bits >> 20) & 1) != 0)
    }
    #[doc = "Bit 21 - AD1TCC3"]
    #[inline(always)]
    pub fn ad1tcc3(&self) -> Ad1tcc3R {
        Ad1tcc3R::new(((self.bits >> 21) & 1) != 0)
    }
    #[doc = "Bit 22 - AD1TCC4"]
    #[inline(always)]
    pub fn ad1tcc4(&self) -> Ad1tcc4R {
        Ad1tcc4R::new(((self.bits >> 22) & 1) != 0)
    }
    #[doc = "Bit 23 - AD1TCPER"]
    #[inline(always)]
    pub fn ad1tcper(&self) -> Ad1tcperR {
        Ad1tcperR::new(((self.bits >> 23) & 1) != 0)
    }
    #[doc = "Bit 24 - AD1TDC2"]
    #[inline(always)]
    pub fn ad1tdc2(&self) -> Ad1tdc2R {
        Ad1tdc2R::new(((self.bits >> 24) & 1) != 0)
    }
    #[doc = "Bit 25 - AD1TDC3"]
    #[inline(always)]
    pub fn ad1tdc3(&self) -> Ad1tdc3R {
        Ad1tdc3R::new(((self.bits >> 25) & 1) != 0)
    }
    #[doc = "Bit 26 - AD1TDC4"]
    #[inline(always)]
    pub fn ad1tdc4(&self) -> Ad1tdc4R {
        Ad1tdc4R::new(((self.bits >> 26) & 1) != 0)
    }
    #[doc = "Bit 27 - AD1TDPER"]
    #[inline(always)]
    pub fn ad1tdper(&self) -> Ad1tdperR {
        Ad1tdperR::new(((self.bits >> 27) & 1) != 0)
    }
    #[doc = "Bit 28 - AD1TEC2"]
    #[inline(always)]
    pub fn ad1tec2(&self) -> Ad1tec2R {
        Ad1tec2R::new(((self.bits >> 28) & 1) != 0)
    }
    #[doc = "Bit 29 - AD1TEC3"]
    #[inline(always)]
    pub fn ad1tec3(&self) -> Ad1tec3R {
        Ad1tec3R::new(((self.bits >> 29) & 1) != 0)
    }
    #[doc = "Bit 30 - AD1TEC4"]
    #[inline(always)]
    pub fn ad1tec4(&self) -> Ad1tec4R {
        Ad1tec4R::new(((self.bits >> 30) & 1) != 0)
    }
    #[doc = "Bit 31 - AD1TEPER"]
    #[inline(always)]
    pub fn ad1teper(&self) -> Ad1teperR {
        Ad1teperR::new(((self.bits >> 31) & 1) != 0)
    }
}
impl W {
    #[doc = "Bit 0 - AD1MC1"]
    #[inline(always)]
    pub fn ad1mc1(&mut self) -> Ad1mc1W<Adc3rSpec> {
        Ad1mc1W::new(self, 0)
    }
    #[doc = "Bit 1 - AD1MC2"]
    #[inline(always)]
    pub fn ad1mc2(&mut self) -> Ad1mc2W<Adc3rSpec> {
        Ad1mc2W::new(self, 1)
    }
    #[doc = "Bit 2 - AD1MC3"]
    #[inline(always)]
    pub fn ad1mc3(&mut self) -> Ad1mc3W<Adc3rSpec> {
        Ad1mc3W::new(self, 2)
    }
    #[doc = "Bit 3 - AD1MC4"]
    #[inline(always)]
    pub fn ad1mc4(&mut self) -> Ad1mc4W<Adc3rSpec> {
        Ad1mc4W::new(self, 3)
    }
    #[doc = "Bit 4 - AD1MPER"]
    #[inline(always)]
    pub fn ad1mper(&mut self) -> Ad1mperW<Adc3rSpec> {
        Ad1mperW::new(self, 4)
    }
    #[doc = "Bit 5 - AD1EEV1"]
    #[inline(always)]
    pub fn ad1eev1(&mut self) -> Ad1eev1W<Adc3rSpec> {
        Ad1eev1W::new(self, 5)
    }
    #[doc = "Bit 6 - AD1EEV2"]
    #[inline(always)]
    pub fn ad1eev2(&mut self) -> Ad1eev2W<Adc3rSpec> {
        Ad1eev2W::new(self, 6)
    }
    #[doc = "Bit 7 - AD1EEV3"]
    #[inline(always)]
    pub fn ad1eev3(&mut self) -> Ad1eev3W<Adc3rSpec> {
        Ad1eev3W::new(self, 7)
    }
    #[doc = "Bit 8 - AD1EEV4"]
    #[inline(always)]
    pub fn ad1eev4(&mut self) -> Ad1eev4W<Adc3rSpec> {
        Ad1eev4W::new(self, 8)
    }
    #[doc = "Bit 9 - AD1EEV5"]
    #[inline(always)]
    pub fn ad1eev5(&mut self) -> Ad1eev5W<Adc3rSpec> {
        Ad1eev5W::new(self, 9)
    }
    #[doc = "Bit 10 - AD1TAC2"]
    #[inline(always)]
    pub fn ad1tac2(&mut self) -> Ad1tac2W<Adc3rSpec> {
        Ad1tac2W::new(self, 10)
    }
    #[doc = "Bit 11 - AD1TAC3"]
    #[inline(always)]
    pub fn ad1tac3(&mut self) -> Ad1tac3W<Adc3rSpec> {
        Ad1tac3W::new(self, 11)
    }
    #[doc = "Bit 12 - AD1TAC4"]
    #[inline(always)]
    pub fn ad1tac4(&mut self) -> Ad1tac4W<Adc3rSpec> {
        Ad1tac4W::new(self, 12)
    }
    #[doc = "Bit 13 - AD1TAPER"]
    #[inline(always)]
    pub fn ad1taper(&mut self) -> Ad1taperW<Adc3rSpec> {
        Ad1taperW::new(self, 13)
    }
    #[doc = "Bit 14 - AD1TARST"]
    #[inline(always)]
    pub fn ad1tarst(&mut self) -> Ad1tarstW<Adc3rSpec> {
        Ad1tarstW::new(self, 14)
    }
    #[doc = "Bit 15 - AD1TBC2"]
    #[inline(always)]
    pub fn ad1tbc2(&mut self) -> Ad1tbc2W<Adc3rSpec> {
        Ad1tbc2W::new(self, 15)
    }
    #[doc = "Bit 16 - AD1TBC3"]
    #[inline(always)]
    pub fn ad1tbc3(&mut self) -> Ad1tbc3W<Adc3rSpec> {
        Ad1tbc3W::new(self, 16)
    }
    #[doc = "Bit 17 - AD1TBC4"]
    #[inline(always)]
    pub fn ad1tbc4(&mut self) -> Ad1tbc4W<Adc3rSpec> {
        Ad1tbc4W::new(self, 17)
    }
    #[doc = "Bit 18 - AD1TBPER"]
    #[inline(always)]
    pub fn ad1tbper(&mut self) -> Ad1tbperW<Adc3rSpec> {
        Ad1tbperW::new(self, 18)
    }
    #[doc = "Bit 19 - AD1TBRST"]
    #[inline(always)]
    pub fn ad1tbrst(&mut self) -> Ad1tbrstW<Adc3rSpec> {
        Ad1tbrstW::new(self, 19)
    }
    #[doc = "Bit 20 - AD1TCC2"]
    #[inline(always)]
    pub fn ad1tcc2(&mut self) -> Ad1tcc2W<Adc3rSpec> {
        Ad1tcc2W::new(self, 20)
    }
    #[doc = "Bit 21 - AD1TCC3"]
    #[inline(always)]
    pub fn ad1tcc3(&mut self) -> Ad1tcc3W<Adc3rSpec> {
        Ad1tcc3W::new(self, 21)
    }
    #[doc = "Bit 22 - AD1TCC4"]
    #[inline(always)]
    pub fn ad1tcc4(&mut self) -> Ad1tcc4W<Adc3rSpec> {
        Ad1tcc4W::new(self, 22)
    }
    #[doc = "Bit 23 - AD1TCPER"]
    #[inline(always)]
    pub fn ad1tcper(&mut self) -> Ad1tcperW<Adc3rSpec> {
        Ad1tcperW::new(self, 23)
    }
    #[doc = "Bit 24 - AD1TDC2"]
    #[inline(always)]
    pub fn ad1tdc2(&mut self) -> Ad1tdc2W<Adc3rSpec> {
        Ad1tdc2W::new(self, 24)
    }
    #[doc = "Bit 25 - AD1TDC3"]
    #[inline(always)]
    pub fn ad1tdc3(&mut self) -> Ad1tdc3W<Adc3rSpec> {
        Ad1tdc3W::new(self, 25)
    }
    #[doc = "Bit 26 - AD1TDC4"]
    #[inline(always)]
    pub fn ad1tdc4(&mut self) -> Ad1tdc4W<Adc3rSpec> {
        Ad1tdc4W::new(self, 26)
    }
    #[doc = "Bit 27 - AD1TDPER"]
    #[inline(always)]
    pub fn ad1tdper(&mut self) -> Ad1tdperW<Adc3rSpec> {
        Ad1tdperW::new(self, 27)
    }
    #[doc = "Bit 28 - AD1TEC2"]
    #[inline(always)]
    pub fn ad1tec2(&mut self) -> Ad1tec2W<Adc3rSpec> {
        Ad1tec2W::new(self, 28)
    }
    #[doc = "Bit 29 - AD1TEC3"]
    #[inline(always)]
    pub fn ad1tec3(&mut self) -> Ad1tec3W<Adc3rSpec> {
        Ad1tec3W::new(self, 29)
    }
    #[doc = "Bit 30 - AD1TEC4"]
    #[inline(always)]
    pub fn ad1tec4(&mut self) -> Ad1tec4W<Adc3rSpec> {
        Ad1tec4W::new(self, 30)
    }
    #[doc = "Bit 31 - AD1TEPER"]
    #[inline(always)]
    pub fn ad1teper(&mut self) -> Ad1teperW<Adc3rSpec> {
        Ad1teperW::new(self, 31)
    }
}
#[doc = "ADC Trigger 3 Register\n\nYou can [`read`](crate::Reg::read) this register and get [`adc3r::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`adc3r::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api)."]
pub struct Adc3rSpec;
impl crate::RegisterSpec for Adc3rSpec {
    type Ux = u32;
}
#[doc = "`read()` method returns [`adc3r::R`](R) reader structure"]
impl crate::Readable for Adc3rSpec {}
#[doc = "`write(|w| ..)` method takes [`adc3r::W`](W) writer structure"]
impl crate::Writable for Adc3rSpec {
    type Safety = crate::Unsafe;
}
#[doc = "`reset()` method sets ADC3R to value 0"]
impl crate::Resettable for Adc3rSpec {}
